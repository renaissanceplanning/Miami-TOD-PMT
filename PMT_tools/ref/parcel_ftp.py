...
# Written by Aaron
def get_ftp_files(folder_connection,
                  ftp_site = "sdrftp03.dor.state.fl.us"):
    '''
...
def florida_counties():
    ...
    fl = ["Alachua","Baker","Bay","Bradford","Brevard","Broward","Calhoun",
          "Charlotte","Citrus","Clay","Collier","Columbia","Miami-Dade",
    ...
    # FIPS
    fips = np.arange(1,24,2).tolist() + [86] + np.arange(27,86,2).tolist() + np.arange(87,134,2).tolist()
    fips = [str(f).zfill(3) for f in fips]
    
    # Iterate through names and create representations
    df = []
    for i in range(len(fl)):
        df.append(pd.DataFrame({"county": fl[i], 
                                "co_no": cono[i],
                                "fips": fips[i],
...
def fdor_gis_and_tax(save_path = None):
    ...
    gis = get_ftp_files(folder_connection = "Map Data",
                        ftp_site = "sdrftp03.dor.state.fl.us")
    ...
    nal = get_ftp_files(folder_connection = "Tax Roll Data Files",
                        ftp_site = "sdrftp03.dor.state.fl.us")
