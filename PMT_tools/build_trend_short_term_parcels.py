...
# %% Main
def prep_short_term_parcels(parcels_path, permits_path, permits_reference_path,
    ...
    PMT.add_unique_id(feature_class=parcels_path)
    fmap = arcpy.FieldMappings()
    fmap.addFieldMap(parcels_path, "ProcessID")

    short_term_parcels = arcpy.FeatureClassToFeatureClass_conversion(in_features=parcels_path,
                                                                     out_path=save_gdb_location,
                                                                     out_name="short_term_parcels", field_mapping=fmap)
    ...
    parcels_arr = arcpy.da.FeatureClassToNumPyArray(in_table=parcels_path, field_names=parcels_fields,
                                                    spatial_reference=parcels_sr, null_value=0)
    parcels = pd.DataFrame(parcels_arr)
    arcpy.DeleteField_management(parcels_path, "ProcessID")
    ...
    permits = pd.DataFrame(permit_array)
    ...
    permits = pd.merge(permits, ref_df, left_on=[lu_match_field, permits_units_field],
    ...
    if units_field_match_dict is not None:
        ...
        permits = pd.merge(permits, ufm, left_on=permits_units_field, right_on=permits_units_field, how="left")

    # calculate the new building square footage for parcel in the permit features u
    # sing the reference table multipliers and overwrites
    #   - calculate those new values.
    #   If a multiplier is given, take
    #       value * multiplier.
    #   Elif,
    #       use the overwrite value.
    #   Else,
    #       return a 0
    print("...applying unit multipliers and overwrites")
    new_living_area = []
    for value, multiplier, overwrite in zip(permits[permits_values_field], permits["Multiplier"], permits["Overwrite"]):
    ...
    permits.drop(columns=["Multiplier", "Overwrite"], inplace=True)

    # update the parcels with the info from the permits
    #   - match building permits to the parcels using id_match_field,
    ...
    # make the replacements. - drop all the rows from the parcels whose IDs are in the permits, - add all the rows
    ...
    parcel_update = parcels[~parcels[id_match_field].isin(pids)]
    ...
    PMT.extendTableDf(in_table=short_term_parcels, table_match_field="ProcessID",
                      df=final_update, df_match_field="ProcessID")

    arcpy.DeleteField_management(in_table=short_term_parcels, drop_field="ProcessID")
