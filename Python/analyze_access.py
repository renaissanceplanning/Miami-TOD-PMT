...
D_ACT_FIELDS = ["TotalJobs", "Consumer", "HealthCare", "EnrollAdlt", "EnrollK12"]
O_ACT_FIELDS = ["HH"]
...
NET_BY_YEAR = {  ##### PUSH THESE SPECS TO A CONFIG OR REFERENCE TABLE?
    2014: {
        "Auto": ["SERPM", BASE_YEAR, HORIZON_YEAR],
        "Transit": ["SERPM", BASE_YEAR, HORIZON_YEAR],
...
def summarizeAccess(skim_table, o_field, d_field, imped_field,
                    se_data, id_field, act_fields, out_fc, ouf_fc_field,
                    imped_breaks, mode, units="minutes",
    ...
    for chunk in pd.read_csv(
        ...
        for i_break in imped_breaks:
            ...
            labels.append(f"{i_break}{units}{mode}")
            low = i_break
        # Apply categories
        chunk[bin_field] = np.select(
            criteria, labels, f"{i_break}{units}p{mode}"
        )
        labels.append(f"{i_break}{units}p{mode}")
        ...
        for act_field in act_fields:
            new_field = f"Wtd{units}{act_field}{mode}"
    ...
    for act_field, prod_field in zip(act_fields, prod_fields):
        avg_field = f"Avg{units}{act_field}{mode}"
    ...
    for year in PMT.YEARS:
        ...
        for mode in MODES:
            ...
            try:
                ...
                copy_ar = arcpy.da.TableToNumPyArray(source, access_fields)
